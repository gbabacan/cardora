      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl max-h-[95vh] md:max-h-[90vh] flex overflow-hidden my-auto">
        ...
        <div className="hidden md:flex w-[500px] bg-[#F7FAFC] border-l border-[#E5EAF0] flex-col flex-shrink-0">
          ...
          <div className="flex-1 flex items-center justify-center p-8">
            ...
            {activeTab === 'image' && previewImage && getImageUrl(previewImage) ? (
              <div className="w-full">
                <div className="w-full aspect-[3/4] rounded-lg overflow-hidden border-2 border-[#E5EAF0]">
                  <img
                ...
                {previewImage.name && (
                  <p className="mt-3 text-sm font-semibold text-center text-[#0B1F2A]">
                    {previewImage.name}
                  </p>
                )}
              </div>
            ) : activeTab === 'image' && !previewImage && (
              <div className="text-center text-[#5B6B75]">
                <svg className="w-16 h-16 mx-auto mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <p className="text-sm">Hover over an image to preview</p>
              </div>
            )}

            {/* Animation and image tabs render their own empty state above. */}
            {(activeTab === 'solid' || activeTab === 'pattern') && !previewColor && !previewPattern && (
              <div className="text-center text-[#5B6B75]">
                <svg className="w-16 h-16 mx-auto mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
